...
say "Setting the model pipeline (deepgram + groq + sarvam)"
...
print(json.dumps({
  ...
  "byok": {
    ...
    "pipeline": {
      ...
      "tts": {"provider":"sarvam","api_key":os.environ["SARVAM_API_KEY"],
        "model":os.environ.get("SARVAM_TTS_MODEL","bulbul:v3"),
        "voice":os.environ.get("SARVAM_TTS_VOICE","shubh"),
...
# ---- d) The Ria receptionist workflow ---------------------------------------
...
print(json.dumps({"name":"Ria Receptionist","workflow_definition":d}))
PY
)
