...
(The hook is price: production voice agents at roughly one rupee, powered by Sarvam Bulbul TTS
(about 20x cheaper than ElevenLabs), with a swappable TTS, LLM, and telephony engine so we are
never locked into one vendor. The voice engine (Sarvam), the brain (Gemini), and telephony
...
`_legacy/server.legacy.js` holds the original verified Sarvam, Gemini, and VoiceLink calls. The
...
| --- | --- | --- |
| **TTS** | `sarvam` | `elevenlabs` (needs `ELEVENLABS_API_KEY`), `sarvam` (needs `SARVAM_API_KEY`) |
...
Public (no auth):
-- `GET  /api/health` returns `{ ok, providers:{ tts:{sarvam}, llm:{gemini}, telephony:{voicelink} }, model }`.
...
-- `POST /api/ws-connect` `{ text, model }` mints a Sarvam streaming session `{ ws_url, token }`.
...
-- **Sarvam browser UA**: Sarvam (`api.sarvam.ai`) sits behind Cloudflare-like protections; the
  server always sends a browser `User-Agent`. Do not remove it.
  Models are `bulbul:v2` and `bulbul:v3` with legacy aliases `muga` and `mulberry`.
  `bulbul:v3` steers with `description` OR `speaker` (`speaker_1..4`) plus `f0_up_key` (clamp -12..12).
  `bulbul:v2` accepts a `[tone]` prefix.
...
returns a real Sarvam WAV, talk-to-it returns a real Gemini reply, telephony status returns the
