...
2. Create a .env file (copy .env.example to .env) and fill in these FOUR keys plus your number.
  ...
  SARVAM_API_KEY=           # the VOICE. Get it at your Sarvam account (dashboard, API key).
...
4. Open http://localhost:8787 in your browser. Click "Launch console", create a tenant,
   ...
   chat, and outbound calling, all on the Sarvam Bulbul voice at roughly one rupee per minute.
...
- **SARVAM_API_KEY** alone gives you the Voice Studio (generate speech). 24kHz studio audio,
  two models: `bulbul:v2` (fast) and `bulbul:v3` (higher quality). Roughly 20x cheaper than ElevenLabs.
...
- For the sub-second Sarvam PHONE agent (Gemini Live brain streamed into Sarvam over the telephony
