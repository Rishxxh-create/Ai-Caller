...
const RATE = { 'bulbul:v2': { disc: 0.99, perm: 5 }, 'bulbul:v3': { disc: 0.50, perm: 2.5 } }; // ₹ / 1000 chars
...
fetch('/api/health').then((r) => r.json()).then((h) => {
  const el = $('#health');
  if (h.ok && h.sarvam && h.gemini) { el.textContent = 'sarvam ✓  brain ✓'; el.className = 'pill ok'; }
  else { el.textContent = 'keys: sarvam ' + (h.sarvam ? '✓' : '✗') + ' brain ' + (h.gemini ? '✓' : '✗'); el.className = 'pill err'; }
...
/* =========================================================
   STUDIO
   ========================================================= */
let studioModel = 'bulbul:v3', tone = 'happy';

function refreshStudioUI() {
  $('#mugaCtl').hidden = studioModel !== 'bulbul:v2';
  $('#mulCtl').hidden = studioModel !== 'bulbul:v3';
...
function studioCharCount() {
  ...
  if (studioModel === 'bulbul:v2') t = `[${tone}] ` + t;
  return t.length;
}
function updateCost() {
...
$('#genBtn').onclick = async () => {
  ...
  if (studioModel === 'bulbul:v2') payload.text = `[${tone}] ` + text;
  if (studioModel === 'bulbul:v3') {
...
/* sample A/B player */
const SAMPLES = [
  ['bulbul:v2 · excited', 'muga_excited.wav', 'tone tag [excited]'],
  ['bulbul:v2 · whisper', 'muga_whisper.wav', 'tone tag [whisper]'],
  ['bulbul:v3 · speaker_1', 'mul_spk1.wav', 'preset studio voice'],
  ['bulbul:v3 · deep male', 'mul_spk3_deep.wav', 'speaker_3 + description'],
  ['bulbul:v3 · pitch +2', 'mul_spk1_pitch.wav', 'f0_up_key = 2'],
  ['bulbul:v3 · streamed', 'stream_mulberry.wav', 'streaming bulbul:v3 WS'],
];
...
(function renderEcon() {
  const data = [
    ['Sarvam bulbul · promo', 5.9, true],
    ['OpenAI tts-1', 15, false],
    ['Deepgram Aura-2', 15, false],
    ['Sarvam bulbul · list', 29, false],
    ['Cartesia Sonic', 35, false],
    ['Sarvam bulbul v2 · list', 59, false],
  ...
  $('#verdictText').innerHTML =
    ...
    "<b>The pricing is the headline.</b> At the launch promo, Sarvam Bulbul v3 at ₹0.50/1k (~$5.9/1M) is cheaper than OpenAI and Deepgram and a fraction of ElevenLabs, and your meter is reading 0 credits used so far. " +
    "At the <b>permanent</b> rate (₹2.5/1k for Bulbul v3, ₹5/1k for Bulbul v2) it's no longer a giveaway: Bulbul v3 lands near Cartesia and under ElevenLabs, but OpenAI/Deepgram undercut it. " +
    "So the move is: <b>build on it now while it's cheap, but design the TTS engine to be swappable</b>, and judge the voice by ear vs ElevenLabs before betting a client product on permanent pricing. For INR-billed Indian voice agents, native rupee pricing + expressiveness is a real edge.";
})();

/* =========================================================
  LIVE , mic → brain → Sarvam streaming TTS
   ========================================================= */
let liveModel = 'bulbul:v3';
$$('#liveModel button').forEach((b) => b.onclick = () => {
  ...
  $('#liveSpeakerWrap').style.opacity = liveModel === 'bulbul:v2' ? .4 : 1;
  $('#latVoice').textContent = liveModel + (liveModel === 'bulbul:v3' ? ' · ' + $('#liveSpeaker').value : '');
});
...
/* speak text through Sarvam streaming WS; resolves when done. */
function speak(text) {
