...
Sarvam Bulbul TTS (about 20x cheaper than ElevenLabs), with a swappable engine so you are
...
The voice engine (Sarvam), the brain (Gemini), and telephony (VoiceLink) are ALREADY
...
```
{
  "tenants": [ { "id":"t_xxx", "name":"Acme Co", "slug":"acme", "createdAt":ISO,
                 ...
                 "providers": { "tts":"sarvam", "llm":"gemini", "telephony":"voicelink" },
                 "plan":"studio" } ],
  ...
  "agents":  [ { "id":"ag_xxx", "tenantId":"t_xxx", "name":"Front Desk",
                 "persona":"...", "tts": { "provider":"sarvam","model":"bulbul:v3","speaker":"anushka","pitch":0 },
...
Public (no auth):
- `GET  /api/health` -> `{ ok, providers: { tts:{sarvam:bool}, llm:{gemini:bool}, telephony:{voicelink:bool} }, model }`
...
- `POST /api/agents` `{ name, persona, tts:{model,speaker,pitch}, greeting, did }` -> `{ agent }`
- `POST /api/agents/update` `{ id, ...fields }` -> `{ agent }` (403 if not this tenant)
- `POST /api/agents/delete` `{ id }` -> `{ ok:true }` (403 if not this tenant)
- `POST /api/tts` `{ text, model, speaker?, pitch?, pace? }` -> `audio/wav` bytes.
  Headers `X-Chars`, `X-Credits-Used`. Increments tenant usage.chars. Reuses verified Sarvam call.
- `POST /api/tts/stream` `{ text, model, speaker? }` -> chunked `audio/wav` for low-latency playback
  (Sarvam streaming synthesis, verified).
...
- **TTS**: `sarvam` (LIVE, reuse exactly: host `api.sarvam.ai`, `api-subscription-key` header,
  `/text-to-speech` and streaming synthesis endpoint, models `bulbul:v2` + `bulbul:v3`, speakers
  such as `anushka` / `shubh` / `meera`, pitch + pace parameters). Stubs (return a clear
  "not configured, add <ENV>" error, `live:false`): `elevenlabs` (needs `ELEVENLABS_API_KEY`),
  `cartesia` (needs `CARTESIA_API_KEY`).
...
3. **Voice Studio** , text in, pick Sarvam model (bulbul:v2 / bulbul:v3), speaker, pitch, pace,
   synthesize (calls /api/tts), waveform + audio player, shows chars + cost. Optional low-latency
   streaming via /api/tts/stream.
...
6. **Settings** , provider registry (/api/providers): show TTS/LLM/Telephony providers, which is active,
   which is ready-to-wire and what env it needs (this is where Zoom/Twilio/ElevenLabs/Claude/Cartesia appear).
...
over 2000 chars (truncate), Sarvam 401/403 on the `api-subscription-key` header (surface upstream detail,
never strip the header), Sarvam non-200 -> surface detail with the request id when present.
...
Server boots clean, /api/health all true, signup+login works, agent create+preview works (real Sarvam WAV),
