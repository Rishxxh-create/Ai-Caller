...
test('registry advertises only implemented providers and keeps Deepgram as sole STT', () => {
  ...
  assert.deepEqual(described.tts.map((row) => row.id), ['sarvam']);
...
test('Sarvam adapter validates its model and synthesizes with mocked HTTP', async () => {
  resetEnv({ SARVAM_API_KEY: 'sarvam-test' });
  let payload;
  const providers = loadProviders(async (_host, _path, _headers, body) => {
    payload = JSON.parse(body.toString('utf8'));
    return { status: 200, headers: {}, buffer: Buffer.from(JSON.stringify({ audios: ['aGVsbG8='] })) };
  });
  const output = await providers.get('tts', 'sarvam').synthesize({ text: 'Hello', model: 'bulbul:v2' });
  assert.equal(payload.model, 'bulbul:v2');
  assert.equal(output.chars, 5);
  assert.throws(
    () => providers.resolveSelection('tts', { provider: 'sarvam', model: 'unknown' }),
