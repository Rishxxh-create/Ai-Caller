...
- The server default TTS is selected by `TTS_PROVIDER` and `TTS_MODEL`. The implemented TTS ID is `sarvam`, with `bulbul:v2` and `bulbul:v3` models. Legacy `muga` and `mulberry` IDs are accepted as compatibility aliases.
